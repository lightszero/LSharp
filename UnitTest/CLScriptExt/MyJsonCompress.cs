...
public class MyJsonBinary
{
    public static void WriteStrDict(System.IO.Stream stream, IList<string> dict)
    {
        ...
        WriteIntSingle(stream, dict.Count);
        foreach (var d in dict)
        {
            var strdata = System.Text.Encoding.UTF8.GetBytes(d);
            WriteIntSingle(stream, strdata.Length);
    ...
    static byte MakeStringTag(bool inDict, bool isPubDict, int keylength)
    {
        ...
        tag |= (byte)(keylength - 1);

        return tag;
    }
    static byte MakeNumberTag(bool isFloat, bool isBool, bool isNull, int datalength)
    {
    ...
    {
        stream.WriteByte(MakeNumberTag(true, false, false, 4));
    ...
    static void WriteIntSingle(System.IO.Stream stream, int number)
    {
        int bytelen = 1;
        int c = number;
        while (c >= 0x100)
